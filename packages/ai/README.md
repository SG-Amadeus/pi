...
#### 3. Model Tables (`src/providers/<id>.models.ts`)

- Fill the provider's model table in `src/providers/<id>.models.ts` manually with the standardized `Model` interface (required fields: `id`, `name`, `api`, `provider`, `baseUrl`, `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`), then re-export it in `src/models.generated.ts`
- Image-generation provider model data is generated by `scripts/generate-image-models.ts` into the standardized `ImagesModel` interface
